$(document).ready(function () {
    ...
    var literatureSlideArray = ['스테이트 오브 테러', '사과는 떨어지지 않는다', '헬프 미 시스터',
                    '용서받지 못한 밤', '오래되고 멋진 클래식 레코드', '우리 모두',
                    '제주는 잘 있습니다', '끝없이 투명해지는 언어', '파친코. 1',
                    '식탁 위의 고백들', '먹는 것과 싸는 것', '인소의 법칙. 4(한정판)(만화)'];

    literatureSlideArray.forEach(function (value, index) {
    ...
    var humanitiesSlideArray = ['카스트', '워터', '내가 틀릴 수도 있습니다',
                    '한없이 가까운 세계와의 포옹', '지구 닦는 황 대리', '어쩌다 마주친 철학',
                    '더 밴드', '우리는 피를 나눈 타인입니다', '다정한 개인주의자',
                    '컬러의 시간', '진실의 조건', '어쩌다 클래식'];

    humanitiesSlideArray.forEach(function (value, index) {
    ...
    var economySlideArray = ['나의 투자는 새벽 4시에 시작된다', '고민의 답', '나답게 일한다는 것',
    ...
    economySlideArray.forEach(function (value, index) {
    ...
    var childSlideArray = ['흔한남매 10', '이상한 과자 가게 전천당 14', '나도 세금 내는 아이가 될래요!',
    ...
    childSlideArray.forEach(function (value, index) {
    ...
    var learningSlideArray = ['2022 선재국어 실전 봉투 모의고사 2', '2023 해커스변호사 민법 맥 암기장', '2022 경찰학 기출 1000',
    ...
    learningSlideArray.forEach(function (value, index) {
    ...
    var foreignSlideArray = ['All the Light We Cannot See', 'Love You By Heart (with StoryPlus QR)', 'The Story Orchestra : The Magic Flute: ',
    ...
    foreignSlideArray.forEach(function (value, index) {
    ...
    // var cdSlideArray = ['최강창민 - Devil [2nd Mini Album][Red Ver.]', '진진&라키(ASTRO) - Restore [1st Mini Album][STAYCATION+VACATION ver.][패키지]', '빅톤(VICTON) - Chronograph [3rd Single Album][Chronos ver. + Graphein ver. SET]',
    ...
    // cdSlideArray.forEach(function (value, index) {
