$(document).ready(function () {
    ...
    var bookDiscountArray = ['자석으로 동물원(41조각)', '피규어 동물백과', '한글 매칭퍼즐', '썼다지웠다: 한글(64개)', '메모리 게임', '썼다지웠다: 미로찾기(64개)', '도미노 게임', '썼다지웠다: 창의미술(64개)'];

    bookDiscountArray.forEach(function (value, index) {
        $.ajax({
            method: "GET",
            url: "https://dapi.kakao.com/v3/search/book?target=title",
            data: {
            ...
            var item = $("<li>", { class: "book_discount" });
            var aTag = $("<a>");
            ...
            $("#book_discount_list").append(item);

        });
    });

    var bookPriceReduyctionArray = ['설민석의 세계사 대모험 정가 인하 세트(1-10권)', '설민석의 한국사 대모험 정가 인하 세트', 'who? 인물 사이언스 정가 인하 세트(1~10권)', 'Go Go 카카오프렌즈 정가 인하 세트', '메모리 게임', '썼다지웠다: 미로찾기(64개)', '도미노 게임', '썼다지웠다: 창의미술(64개)'];

    bookPriceReduyctionArray.forEach(function (value, index) {
        $.ajax({
            method: "GET",
            url: "https://dapi.kakao.com/v3/search/book?target=title",
            data: {
            ...
            $("#book_price_reduction_list").append(item);
